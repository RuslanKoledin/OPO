...
@bot.message_handler(func=lambda message: message.text in ["1. Проблемы с почтой",
                                                           ...
                                                           "5. Монитор не включается"])
def handle_main_problem(message):
    """Обрабатывает выбор основной проблемы."""
    # УДАЛЕНА ПРОБЛЕМНАЯ СТРОКА user_data[message.chat.id] = {}
    ...
    elif problem_text == "4. Настройка прокси Windows":
        ask_windows_subproblem(message)
    elif problem_text == "3. Не работает наушник - звук/микрофон":
        ask_head_seat_subproblem(message)
    elif problem_text == "5. Монитор не включается":
        ask_monitor_subproblem(message)
...
def ask_monitor_subproblem(message):
    """Спрашивает под-проблему с монитором."""
    keyboard = create_keyboard([
...
def handle_monitor_subproblem(message):
    subproblem_text = message.text
    user_data[message.chat.id]['problem'] = subproblem_text
    user_data[message.chat.id]['detailed_problem'] = subproblem_text
